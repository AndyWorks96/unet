...
file_path01 = './data/dcm'
file_path02='./data/DWI_ROI'
...
def getPatientDir():
    ...
    for subsetindex in range(len(path_list01)):
        retal_subset_path01 = file_path01 + "/" + str(path_list01[subsetindex]) + "/"+"DWI/"
        ...
        OnePeople = np.zeros([patientLen, 256, 256], dtype=np.uint8)
        OneMask = np.zeros([patientLen, 256, 256], dtype=np.uint8)
        ...
        savedir = './data/dwiTest/'+ str(path_list01[subsetindex]) + '/'
        sitk.WriteImage(saveOnePeople, savedir + str(path_list01[subsetindex]) + "_dwi.nii.gz")
