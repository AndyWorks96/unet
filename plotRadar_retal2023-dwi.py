...
labels=np.array(['Dice','Sensitivity', 'Specificity'])
nAttr=3
labels=np.concatenate((labels,[labels[0]]))  #对labels进行封闭
data_1=np.array([0.754,0.798,0.987])
Chen = np.array([0.806,0.835,0.991])
Sun = np.array([0.822,0.874,0.993])


Cheng = np.array([0.851,0.882,0.990])
Zhang = np.array([0.867,0.88,0.975])
Kotowski  = np.array([0.830,0.81,0.985])
Mostefa = np.array([0.840,0.84,0.983])
...
plt.plot(angles,data_1,'bo-',color='r',linewidth=2, label='FCN',MarkerSize = 5)
plt.fill(angles,data_1,facecolor='r',alpha=0.2)

Chen = np.concatenate((Chen, [Chen[0]]))
plt.plot(angles,Chen,'o-',color='g',linewidth=0.8, label='Att-U-Net',MarkerSize = 4)
plt.fill(angles,Chen,facecolor='g',alpha=0.2)

Sun = np.concatenate((Sun, [Sun[0]]))
plt.plot(angles,Sun,'p-',color='pink',linewidth=0.8, label='Nas-U-Net',MarkerSize = 4)
plt.fill(angles,Sun,facecolor='pink',alpha=0.2)

Cheng = np.concatenate((Cheng, [Cheng[0]]))
plt.plot(angles,Cheng,'s-',color='lime',linewidth=0.8, label='Ours',MarkerSize = 4)
...
plt.savefig('dota_radar.svg', dpi=300)
plt.show()
...
# plt.grid(True)
# plt.legend(loc=1, bbox_to_anchor=(1.45, 1.1))
# plt.savefig('data_radar_english.svg', dpi=300)
# plt.show()
