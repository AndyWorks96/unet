...
def getPaRadio():
    ...
    all_data_frame = pd.DataFrame(list(dict_item01))[22:122]
    all_data_frame.loc['123'] = ['name', 'REN GUI LIAN^REN GUI' ]
    for subsetindex  in range(len(path_list)):
        if subsetindex == 0:
            continue
        retal_subset_path = file_path + "/" + str(path_list[subsetindex])+"/"
        imageName = retal_subset_path +str(path_list[subsetindex])+"_dwi.nii.gz"
        ...
        data_frame.loc['123'] = ['name',path_list[subsetindex] ]
