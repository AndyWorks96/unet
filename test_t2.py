...
from dataset_t2 import Dataset
import unet_5k
import unet
...
def main():
    ...
    img_paths = glob('./data/t2Image01/*')
    mask_paths = glob('./data/t2Mask01/*')
    ...
    model.load_state_dict(torch.load('models/%s/model_t2.pth' % args.name, map_location=lambda storage, loc: storage))
    model.eval()
        ...
        with warnings.catch_warnings():
            ...
            with torch.no_grad():
                for i, (input, target) in tqdm(enumerate(val_loader), total=len(val_loader)):
                    ...
                    # 提取模型预测出来的数组
                    # test_output = np.transpose(output, (1, 0, 2, 3))  # (3,18,240,240)
                    # for mode_idx in range(test_output.shape[0]):
                    #     for slice_idx in range(test_output.shape[1]):
                    #         prediction[slice_idx + i * 18][mode_idx] = test_output[mode_idx][slice_idx]
