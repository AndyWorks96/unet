...
from dataset_dwi import Dataset
import unet_5k
import unet
...
def main():
    ...
    img_paths = glob('./data/dwiImage01/*')
    mask_paths = glob('./data/dwiMask01/*')
    ...
    model.load_state_dict(torch.load('models/%s/model_dwi.pth' % args.name, map_location=lambda storage, loc: storage))
    model.eval()
            ...
            with torch.no_grad():
                for i, (input, target) in tqdm(enumerate(val_loader), total=len(val_loader)):
                    ...
                    for i in range(output.shape[0]):
                        ...
                        rgbName = rgbName  + ".png"
                        rgbPic = np.zeros([224, 224, 1], dtype=np.uint8)
