...
class CNNModelTrains(object):
    def __init__(self, model_save='', features_save='', img_size=[26,380,380, 1], n_filters=32, n_features=50):
    ...
    def cnnModel(self, img_size, n_filters, n_features):
        img_x, img_y, img_z, img_a = img_size[0], img_size[1], img_size[2],img_size[3]

        inputs = Input((img_x, img_y, img_z,img_a))
        ...
        conv1_3d = Conv3D(64, (1, 3, 3), strides=1, activation='relu', use_bias=True, padding='same',kernel_initializer='he_normal')(inputs)
        conv1_3d = Conv3D(64, (1, 3, 3), strides=1, activation='relu', use_bias=True, padding='same',kernel_initializer='he_normal')(conv1_3d)
    ...
    def loadData(self, train_path, label_path):
        ...
        # img_paths_3d = glob('H:/李星项目/brats_feature/DataSets/seg_model/dwiImage3d/*')
        # mask_paths = glob('H:/李星项目/brats_feature/DataSets/seg_model/dwiMask01/*')
        img_paths_3d = glob('H:/李星项目/brats_feature/DataSets/seg_model/t2Image3d/*')
        mask_paths = glob('H:/李星项目/brats_feature/DataSets/seg_model/t2Image3d/*')
        xlsx1_filePath = 'H:/李星项目/brats_feature/DataSets/Rectal Outcome.xlsx'
        images = np.zeros([411, 1, 224, 224], dtype=np.float64)
        images_3d = np.zeros([43,1, 26, 380, 380], dtype=np.float64)

        # images_3d = images_3d.transpose((2, 0, 1))

        # masks = np.zeros([411, 1], dtype=np.float64)
        data_1 = pd.read_excel(xlsx1_filePath)
        masks_3d = np.zeros([43, 1], dtype=np.float64)
        img_name_list = []
        img3d_name_list = []
        print(len(images))
        ...
        for i in range(len(images_3d)):
            ...
            img1_name = img1[51:-4]
            img3d_name_list.append(img1_name)
            npimage = np.load(img_paths_3d[i])
            ...
            for ix in range(43):
                d1 = data[ix][0]
                if data[ix][0] == img1_name:
                        masks_3d[i][0] = data[ix][1]

            for idx in range(images_3d.shape[2]):
                for idy in range(images_3d.shape[3]):
                    for idz in range(images_3d.shape[4]):
                        images_3d[i, 0, idx, idy,idz] = npimage[0, idx, idy,idz]
        # 保存到本地excel
        ...
        # labels = np.load(label_path)
        trains = images_3d
        trains = trains.transpose(0,2,3,4,1)
        labels = masks_3d
        x_train, x_test, y_train, y_test = train_test_split(trains, labels, test_size=0.1)
...
if __name__ == '__main__':
    ...
    cnn_model_train = CNNModelTrains(model_save, features_save, img_size=[26, 380, 380,1], n_features=960, n_filters=16)
    cnn_model_train.fitModel(train_path, label_path)



    model, feature_model = cnn_model_train.cnnModel(img_size=[26, 380, 380, 1], n_features=960, n_filters=32)
