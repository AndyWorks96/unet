...
def train(args, train_loader, model, criterion, optimizer, epoch, scheduler=None):
    ...
    #     target = target.cuda()
        ...
        # compute gradient and do optimizing step  计算梯度并执行优化步骤{'lr': 0.0003, 'betas': (0.9, 0.999), 'eps': 1e-08, 'weight_decay': 0, 'amsgrad': False}
...
def main():
    ...
    train_img_paths, val_img_paths, train_mask_paths, val_mask_paths = \
        train_test_split(img_paths, mask_paths, test_size=0.2, random_state=0)
    ...
    # model = attention_Unet.__dict__[args.arch](args)
    # model = UnetNotSymmetry.__dict__[args.arch](args)

    model = unet.__dict__[args.arch](args)
    # model=attention_Unet.AttU_Net(4,3)
    # model = model.cuda()
    # solution: 0
    device = 'cpu'
    ...
    for epoch in range(args.epochs):
        ...
        if val_log['iou'] > best_iou:
            torch.save(model.state_dict(), 'models/%s/model_dwi.pth' %args.name)
